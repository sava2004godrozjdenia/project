...
;(function($) {
  ...
  var cartData,
    ...
    totalCnt = 0,
    visibleLabel = false,
    label = $('<div class="jqcart-cart-label"><span class="jqcart-title">Корзина</span><span class="jqcart-total-cnt">0</span></div>'),
    modal = '<div class="jqcart-layout"><div class="jqcart-checkout">123</div></div>',
    orderform = '<p class="jqcart-cart-title">Contact Information:</p><form class="jqcart-orderform"><p><label>Name:</label><input type="text" name="user_name"></p><p><label>Phone:</label><input type="text" name="user_phone"></p><p><label>Email:</label><input type="text" name="user_mail"></p><p><label>Address:</label><input type="text" name="user_address"></p><p><label>A comment:</label><textarea name="user_comment"></textarea></p><p><input type="submit" value="Make an order"><input type="reset" value="Back to shopping"></p></form>';
  var opts = {
		buttons: '.add_item',
		cartLabel: 'body',
		visibleLabel: false,
  ...
  var actions = {
    ...
    openCart: function() {
      var subtotal = 0,
			cartHtml = '';
      cartData = actions.getStorage();
      orderPreview = '<p class="jqcart-cart-title">Shopping basket <span class="jqcart-print-order"></span></p><div class="jqcart-table-wrapper"><div class="jqcart-manage-order"><div class="jqcart-thead"><div>ID</div><div></div><div>Goods</div><div>Price</div><div>Quantity</div><div>Amount</div><div></div></div>';
      var key, sum = 0;
      for (key in cartData) {
        if (cartData.hasOwnProperty(key)) {
      ...
      orderPreview += '<div class="jqcart-subtotal">Total price: <strong>' + subtotal + '</strong> ' + opts.currency + '</div>';
			
			cartHtml = subtotal ? (orderPreview + orderform) : '<h2 class="jqcart-empty-cart">Cart empty</h2>';
    ...
    sendOrder: function(e) {
      ...
      if ($.trim($('[name=user_name]', $that).val()) === '' || $.trim($('[name=user_phone]', $that).val()) === '') {
        $('<p class="jqcart-error">Please enter your name and phone number!</p>').insertBefore($that).delay(3000).fadeOut();
        return false;
      }
      $.ajax({
        url: opts.handler,
